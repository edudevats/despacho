...
# The inventory functions are dynamically placed into inventory blueprint. Let's load them from the analyzer output or just check all available functions.
# Instead of hardcoding inventory, if a function isn't in the above dicts, we assume it's 'inventory'. But wait! Flask static files use `url_for('static', filename=...)`. So 'static' should be ignored.
